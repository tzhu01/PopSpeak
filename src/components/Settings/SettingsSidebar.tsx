...
export function SettingsSidebar({ activePane, onSelect }: Props) {
  ...
  return (
    <div className="settings-sidebar w-[188px] h-full shrink-0 jelly-surface-flat border border-border rounded-[20px] flex flex-col py-4 px-2 gap-1 overflow-hidden">
      <div className="settings-sidebar-heading px-3 pb-4">
        <p className="brand-kicker mb-1">CONTROL</p>
        <h2 className="brand-display text-[17px] font-semibold text-text-primary">
          {t('settings.title')}
        </h2>
      </div>
      {PANES.map((pane) => {
        ...
        return (
          <motion.button
            ...
            className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-[10px] text-[13px] border-none cursor-pointer transition-colors text-left relative ${
              isActive
