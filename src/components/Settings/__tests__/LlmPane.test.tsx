...
const mockAppStore = {
  config: {
    llm_provider: 'openrouter' as string,
    llm_api_key: '',
...
describe('LlmPane', () => {
  ...
  describe('Provider selection', () => {
    it('renders provider dropdown with current value', () => {
      ...
      expect(
        [...(providerSelect as HTMLSelectElement).options].map((option) => option.text),
      ).toEqual(['本地文字润色', 'Ollama（外部本地服务）', 'OpenRouter', 'PopSpeak Cloud'])
    })
  ...
  describe('Base URL input', () => {
    it('updates config when base URL changes', () => {
      render(<LlmPane />)
      const input = screen.getByPlaceholderText('https://openrouter.ai/api/v1')
  ...
  describe('Managed local polish UI', () => {
    ...
    it('auto-indexes bundled resources without exposing URL, port, or absolute paths', async () => {
      ...
      vi.mocked(tauri.getLocalLlmPaths).mockResolvedValue({
        server_path: String.raw`D:\portable\runtimes\llama\llama-server.exe`,
        model_dir: String.raw`\\?\D:\portable\models\llm`,
        default_model_path: String.raw`D:\portable\models\llm\qwen.gguf`,
        default_model_ready: true,
        upgrade_model_path: String.raw`D:\portable\models\llm\upgrade.gguf`,
        upgrade_model_ready: false,
      })
      vi.mocked(tauri.localLlmHealth).mockResolvedValue(true)

      const { container } = render(<LlmPane />)

      expect(await screen.findByText('本地润色服务运行中')).toBeInTheDocument()
      expect(screen.queryByText('API Key')).not.toBeInTheDocument()
      expect(screen.queryByText('Base URL')).not.toBeInTheDocument()
      expect(screen.queryByPlaceholderText('e.g. gpt-4o-mini')).not.toBeInTheDocument()
      expect(container.querySelector('input[type="number"]')).toBeNull()
      expect(container.textContent).not.toContain('D:\\portable')
      expect(
        screen.getByText('对语音识别结果做文字整理，尝试去除口癖、重复字并修正明显错字。'),
      ).toBeInTheDocument()
      expect(screen.getByText(/无需填写路径、URL 或端口/)).toBeInTheDocument()
      ...
      expect(ordinaryView.textContent).not.toMatch(/Qwen|Q4_K_M|models\/llm|runtimes\/llama/i)
      expect(screen.getByText(/Qwen2.5-0.5B-Instruct/).closest('details')).not.toHaveAttribute(
        'open',
      )
    })
  })
